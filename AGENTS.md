...
**Concern OS Existing-Pane Input:** Stage 1 attaches to tmux-owned Amplifier panes without creating or resuming processes. Initial capture validates the exact configured tmux socket plus tmux-server, pane, and Amplifier PID generations and issues a short-lived generation-bound lease. Focused browser polling renews the lease through full validation. Input is coalesced for one frame, delivered FIFO with monotonic replay rejection, revalidates all `/proc` generations immediately before an atomic tmux identity-check-plus-`send-keys -H` transaction, and fails closed on stale identity. Never replace this path with shell interpolation, a bare/default tmux socket, name-based targeting, or per-keystroke catalog reconciliation.
